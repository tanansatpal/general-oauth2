...
function OAuth2Strategy(options, verify) {
  ...
  // NOTE: _redirectUrlAsName option is provided for EBAY support
  this._redirectUrlAsName = options.redirectUrlAsName ? options.redirectUrlAsName : false;
  this._saveQueryParamsToProfile = options.saveQueryParamsToProfile ? options.saveQueryParamsToProfile : false;
...
OAuth2Strategy.prototype.authenticate = function (req, options) {
  ...
  if (req.query && req.query.code) {
    function loaded(err, ok, state) {
      ...
      if (self._saveQueryParamsToProfile) {
        self._authQuery = {};
        for (var key in req.query) {
          if (req.query.hasOwnProperty(key)) {
            self._authQuery[key] = req.query[key];
          }
        }
      }
...
OAuth2Strategy.prototype.userProfile = function (accessToken, done) {
  var profile = {};
  if (this._authQuery) {
    profile = Object.assign(profile, this._authQuery);
  }
